...
const AppointRow = ({ appoint, index, refetch }) => {
  ...
  const { _id, name, date, email, status } = appoint;
        ...
        axiosSecure
          ...
          .then((data) => {
            if (data.data.modifiedCount > 0) {
              Swal.fire({
                title: "Canceled",
                text: "Your test has been Canceled.",
                icon: "success",
              });
              refetch();
            }
  ...
  return (
    <>
      <tr>
        ...
        <td>{date}</td>
        <td>{email}</td>
        <td><span className="text-red-600 bg-[#2EE2B5] py-2 rounded-full px-4 font-bold" >{status}</span></td>
        <td>
          <button
